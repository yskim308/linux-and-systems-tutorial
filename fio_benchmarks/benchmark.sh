...
MON_PID=$!
fio --name=seq_write_async --rw=write --ioengine=io_uring --iodepth=32 --direct=1 --bs=4k --size=$TEST_SIZE --filename=$TEST_FILE --output-format=json --output=results/raw/seq_write_async.json
sleep 1
kill $MON_PID
clear_caches
...
MON_PID=$!
fio --name=seq_read_async --rw=read --ioengine=io_uring --iodepth=32 --direct=1 --bs=4k --size=$TEST_SIZE --filename=$TEST_FILE --output-format=json --output=results/raw/seq_read_async.json
sleep 1
kill $MON_PID
clear_caches
