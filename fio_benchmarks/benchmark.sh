...
# Function to clear caches quickly between runs
clear_caches() {
    sync
    if [ "$EUID" -eq 0 ]; then
...
iostat -xyz 0.5 > results/raw/seq_write_sync_iostat.log &
MON_PID=$!
sleep 1
...
iostat -xyz 0.5 > results/raw/seq_write_async_iostat.log &
MON_PID=$!
sleep 1
fio --name=seq_write_async --rw=write --ioengine=io_uring --iodepth=32 --direct=1 --bs=128k --size=2G --filename=testfile.img --output-format=json --output=results/raw/seq_write_async.json
sleep 1
kill $MON_PID
clear_caches

echo "Running: Sequential Read (Sync)..."
iostat -xyz 0.5 > results/raw/seq_read_sync_iostat.log &
MON_PID=$!
sleep 1
fio --name=seq_read_sync --rw=read --ioengine=sync --direct=1 --bs=128k --size=2G --filename=testfile.img --output-format=json --output=results/raw/seq_read_sync.json
sleep 1
kill $MON_PID
clear_caches

echo "Running: Sequential Read (Async)..."
iostat -xyz 0.5 > results/raw/seq_read_async_iostat.log &
MON_PID=$!
sleep 1
...
iostat -xyz 0.5 > results/raw/rand_write_sync_iostat.log &
MON_PID=$!
sleep 1
fio --name=rand_write_sync --rw=randwrite --ioengine=sync --direct=1 --bs=4k --size=2G --filename=testfile.img --output-format=json --output=results/raw/rand_write_sync.json
sleep 1
kill $MON_PID
clear_caches

echo "Running: Random Write (Async)..."
iostat -xyz 0.5 > results/raw/rand_write_async_iostat.log &
MON_PID=$!
sleep 1
...
iostat -xyz 0.5 > results/raw/rand_read_sync_iostat.log &
MON_PID=$!
sleep 1
fio --name=rand_read_sync --rw=randread --ioengine=sync --direct=1 --bs=4k --size=2G --filename=testfile.img --output-format=json --output=results/raw/rand_read_sync.json
sleep 1
kill $MON_PID
clear_caches

echo "Running: Random Read (Async)..."
iostat -xyz 0.5 > results/raw/rand_read_async_iostat.log &
MON_PID=$!
sleep 1
...
clear_caches
iostat -xyz 0.5 > results/raw/seq_to_rand_iostat.log &
MON_PID=$!
sleep 1
